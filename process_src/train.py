...
train_temp_path = './process_file/train_temp.csv'
out_dir = './ava_finally/annotations/train.csv'
train_temp = []
        ...
        if videoName == data[0]:
            if index2 == index:
                ...
                for index3 in range(10):
                    if index + index3 + 1 >= len(train_temp):
                        continue
                    if train_temp[index + index3 + 1][-1] == '-1':
                        ...
                        if abs(x1 - xT1) < 0.01 and abs(y1 - yT1) < 0.01 and abs(x2 - xT2) < 0.01 and abs(
                                y2 - yT2) < 0.01:
                ...
                if abs(x1 - xTT1) < 0.005 and abs(y1 - yTT1) < 0.005 and abs(x2 - xTT2) < 0.005 and abs(
                        y2 - yTT2) < 0.005:
                    continue
